...
import {
  ...
  Clock, Gauge, Key, Zap, Disc3, User, CheckCircle, X, ArrowRight,
  Music2, Layers, XCircle, Shield
} from "lucide-react";
...
const SimilarTrackRow = ({ track, isActive, isPlaying, onPlay, onLike }: {
  track: any;
  isActive: boolean;
  isPlaying: boolean;
  onPlay: () => void;
...
export default function TrackDetailPage() {
  ...
  const { playTrack, activeTrack, isPlaying, togglePlay } = usePlayerStore();
  const { tracks: MUSIC_TRACKS } = useTrackStore();
  ...
  const similarTracks = useMemo(() => {
    if (!track) return [];
    return MUSIC_TRACKS
      .filter(t => t.id !== track.id && t.genre.some((g: string) => track.genre.includes(g)))
      .slice(0, 5);
  }, [track]);
  ...
  if (!track) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#060606] text-white">
        <div className="w-20 h-20 rounded-3xl bg-white/[0.04] flex items-center justify-center mb-6">
          <Disc3 size={32} className="text-white/30" />
        </div>
        <h1 className="text-2xl font-bold mb-2">Track Not Found</h1>
        <p className="text-white/40 mb-8">This track might have been removed or renamed.</p>
        <button
          onClick={() => router.push("/library/royalty-free-music")}
          className="px-6 py-3 bg-white text-black rounded-full font-bold text-xs uppercase tracking-wider hover:scale-105 transition-all"
        >
          Back to Library
        </button>
      </div>
